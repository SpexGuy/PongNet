...
#define ballWidth 6
#define ballHeight 6
static unsigned char ball_raster[28];
...
static unsigned char paddle_sprite_top_raster[65];
...
static unsigned char paddle_short_top_raster[35];
...
static unsigned char paddle_long_top_raster[95];
...
static unsigned char paddle_sprite_bottom_raster[65];
...
static unsigned char paddle_short_bottom_raster[35];
...
static unsigned char paddle_long_bottom_raster[95];
...
static unsigned char fastball_raster[40];
...
#define longWidth 8
#define longHeight 7
static unsigned char long_raster[40];
...
static unsigned char short_raster[40];
...
static unsigned char multiball_raster[40];
...
#define spaceInvadersWidth 8
#define spaceInvadersHeight 7
static unsigned char spaceInvaders_raster[40];
static unsigned char spaceInvaders_data[] = 
{ 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
...
#define PONG 1
#define SPACE_INVADERS 2

#define SPACE_GAME_LENGTH 10000
...
void 	update_spaceInvaders(void);
...
void	updateScreen_pong(void);
void	updateScreen_spaceInvaders(void);
void	updateSpacePower(void);
void	updateColl(int player);
void	initAliens(void);
void	drawSprite(Sprite *s, float x, float y);
unsigned char getColor(Sprite *s, int xc, int yc, float xf, float yf, bool even);
...
int main(void) {
	...
	players[0].paddle.midSprite.raster = paddle_sprite_top_raster;
	...
	players[0].paddle.shortSprite.raster = paddle_short_top_raster;
	...
	players[0].paddle.longSprite.raster = paddle_long_top_raster;
	...
	players[1].paddle.midSprite.raster = paddle_sprite_bottom_raster;
	...
	players[1].paddle.shortSprite.raster = paddle_short_bottom_raster;
	...
	players[1].paddle.longSprite.raster = paddle_long_bottom_raster;
	...
	if (read_Switches() & 0x04) {
		...
		srand(readPotentiometer0() ^ readPotentiometer1() ^ readPotentiometer2());
	}
	...
	powers[MULTIBALL].sprite.raster = multiball_raster;
	...
	powers[FASTBALL].sprite.raster = fastball_raster;
	...
	powers[LONG_PADDLE].sprite.raster = long_raster;
	...
	powers[SHORT_PADDLE].sprite.raster = short_raster;
	
	powers[SPACE_INVADERS_POWER].active = FALSE;
	powers[SPACE_INVADERS_POWER].available = TRUE;
	powers[SPACE_INVADERS_POWER].prb = (int) (0.1 * RAND_MAX);
	powers[SPACE_INVADERS_POWER].val = 10000;
	powers[SPACE_INVADERS_POWER].decr = TRUE;
	
	//--CHANGE THE DAMN SPRITE--//
	powers[SPACE_INVADERS_POWER].sprite.data = spaceInvaders_data;
	powers[SPACE_INVADERS_POWER].sprite.height = spaceInvadersHeight;
	powers[SPACE_INVADERS_POWER].sprite.width = shortWidth;
	powers[SPACE_INVADERS_POWER].sprite.yOffset = shortHeight/2;
	powers[SPACE_INVADERS_POWER].sprite.xOffset = shortWidth/2;
	powers[SPACE_INVADERS_POWER].sprite.raster = short_raster;
	...
	for (c = 0; c < NUM_BALLS; c++) {
		...
		balls[c].sprite.raster = ball_raster;
	}
	
	for (c = 0; c < NUM_BULLETS; c++) {
		bullets[c].active = FALSE;
		bullets[c].sprite.data = ball;
		bullets[c].sprite.height = ballHeight;
		bullets[c].sprite.width = ballWidth;
		bullets[c].sprite.xOffset = ballWidth/2;
		bullets[c].sprite.yOffset = ballHeight/2;
		bullets[c].sprite.raster = ball_raster;
	}
	
	for (c = 0; c < NUM_ALIENS; c++) {
		aliens[c].active = FALSE;
		//Use random types
		aliens[c].type = rand() % 4;
		aliens[c].sprite.data = multiball_data;
	}
	
	time = 0;
	reset(TRUE);
	
	scoreboard = read_Switches() & 0x08;
	...
	while(1) {
		if(game_mode == PONG) {
			if(players[0].powerStatus[SPACE_INVADERS_POWER] == 0 ||
					players[1].powerStatus[SPACE_INVADERS_POWER] == 0)
			update_pong();
		} else {
			update_spaceInvaders();
		}
		
		time++;
		if (read_Switches() & 0x10)
...
void update_spaceInvaders() {
	updateSpaceship();
	updateAliens();
	updateBullets();
	updateSpacePower();
	updateScreen_spaceInvaders();
	
	if(space_game_time-- <= 0)
		change_game_mode();
}

void updateSpacePower() {
		if (powers[SPACE_INVADERS_POWER].decr) {
		if (players[0].powerStatus[SPACE_INVADERS_POWER] != 0) {
			players[0].powerStatus[SPACE_INVADERS_POWER]--;
			if (players[0].powerStatus[SPACE_INVADERS_POWER] == 0) {
				powers[SPACE_INVADERS_POWER].available = TRUE;
				updateColl(0);
			}
		}
		if (players[1].powerStatus[SPACE_INVADERS_POWER] != 0) {
			players[1].powerStatus[SPACE_INVADERS_POWER]--;
			if (players[1].powerStatus[SPACE_INVADERS_POWER] == 0) {
				powers[SPACE_INVADERS_POWER].available = TRUE;
				updateColl(1);
			}
...
void change_game_mode() {
	if(game_mode == PONG) {
		space_game_time = SPACE_GAME_LENGTH;
...
void updateBullets() {
	...
	for(c = 0; c < NUM_BULLETS; c++) {
		...
		for(d = 0; d < NUM_ALIENS; d++) {
			if(aliens[d].x >= b->x && aliens[d].x + ALIEN_WIDTH <= b->x) {
				if(aliens[d].y >= lastY && aliens[d].y + ALIEN_HEIGHT <= b->y) {
					b->active = FALSE;
					aliens[d].active = FALSE;
					if(current_player)
						scoreP1();
					else
						scoreP2();
...
void initAliens() {
	...
	for(x = 0; x < SCREEN_WIDTH; x = x + ALIEN_WIDTH) {
		for(y = 0; y < ALIEN_HEIGHT * 4; y++) {
			aliens[x*y].active = TRUE;
			aliens[x*y].x = x;
			aliens[x*y].y = y;
		}
	}
		...
		if (players[c].ai) {
			if (players[c].paddle.collTime > 0)
				players[c].paddle.x += clamp(
							(players[c].paddle.collX - players[c].paddle.x)/(float)players[c].paddle.collTime--,
							-MAX_PADDLE_SPEED, MAX_PADDLE_SPEED);
		} else {
			players[c].paddle.x = (float)(SCREEN_WIDTH - paddleWidth(c)) * (float)(c == 0 ? readPotentiometer0() : readPotentiometer2()) / (float)POTEN_MAX;
		}
	}
}
...
void updateSpaceship() {
	if(players[current_player].ai) {
		...
		players[current_player].paddle.x = (float)(SCREEN_WIDTH - paddleWidth(current_player)) * (float)(current_player == 0 ? readPotentiometer0() : readPotentiometer2()) / (float)POTEN_MAX;
	}
}

void updateScreen_pong() {
	char str[40];
	int c;
...
void updateScreen_spaceInvaders() {
//do this

}

void drawSprite(Sprite *s, float x, float y) {
	int xPos = (int)(x - s->xOffset);
	int yPos = (int)(y - s->yOffset);
	float xf = (x - s->xOffset) - xPos;
	float yf = (y - s->yOffset) - yPos;
	bool even = xPos%2 == 0;
	int xc, yc;
	for (xc = 0; xc <= s->width/2; xc++) {
		for (yc = 0; yc <= s->height; yc++) {
			s->raster[yc*(s->width/2+1)+xc] = getColor(s, xc, yc, xf, yf, even);
		}
	}
	RIT128x96x4ImageDraw(s->raster, xPos, yPos, s->width+2, s->height+1);
}
...
unsigned char getColor(Sprite *s, int xc, int yc, float xf, float yf, bool even) {
	...
	unsigned char pix;
	if (even) {
		if (xc-1 >= 0 && yc < s->height) {
			pix = s->data[yc*s->width/2 + xc-1];
			color1 += yf*(1-xf)*(pix&0x0F);
		}
		if (yc-1 >= 0 && xc < s->width/2) {
			pix = s->data[(yc-1)*s->width/2 + xc];
			color0 += (1-yf)*(1-xf)*(pix>>4);
			color0 += (1-yf)*xf*(pix&0x0F);
			color1 += (1-yf)*xf*(pix>>4);
		}
		if (yc-1 >= 0 && xc-1 >= 0) {
			pix = s->data[(yc-1)*s->width/2 + xc-1];
			color1 += (1-yf)*(1-xf)*(pix&0x0F);
		}
		if (yc < s->height && xc < s->width/2) {
			pix = s->data[yc*s->width/2 + xc];
			color0 += yf*(1-xf)*(pix>>4);
			color0 += yf*xf*(pix&0x0F);
			color1 += yf*xf*(pix>>4);
		}
	} else {
		if (xc-1 >= 0 && yc < s->height) {
			pix = s->data[yc*s->width/2 + xc-1];
			color1 += yf*(1-xf)*(pix>>4);
			color1 += yf*xf*(pix&0x0F);
			color0 += yf*(1-xf)*(pix&0x0F);
		}
		if (yc-1 >= 0 && xc < s->width/2) {
			pix = s->data[(yc-1)*s->width/2 + xc];
			color0 += (1-yf)*xf*(pix>>4);
		}
		if (yc-1 >= 0 && xc-1 >= 0) {
			pix = s->data[(yc-1)*s->width/2 + xc-1];
			color1 += (1-yf)*(1-xf)*(pix>>4);
			color1 += (1-yf)*xf*(pix&0x0F);
			color0 += (1-yf)*(1-xf)*(pix&0x0F);
		}
		if (yc < s->height && xc < s->width/2) {
			pix = s->data[yc*s->width/2 + xc];
			color0 += yf*xf*(pix>>4);
	...
	return (((unsigned char)color1) << 4) | ((unsigned char)color0);
}
