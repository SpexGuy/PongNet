...
// 
// NAME:		    Martin Wickham (303),
//							Scott Hendrickson (304),
//							Kody Greenbaum (304)
//
...
/* A Player contains a Paddle and keeps track of powerups */
...
/* A Powerup may be placed on the field */
...
//the loop count
int time;
//the statistics for the scoreboard
char game_stats[6];
//whether the scoreboard is in use
bool scoreboard;
...
int main(void) {
	...
	//set up player 0
	players[0].ai = read_Switches() & 0x0002; //check switch 4
	players[0].points = 0;
	players[0].paddle.y = PADDLE1_Y;

	//set up player 0 sprites
	...
	//set up player 1
	players[1].ai = read_Switches() & 0x0001; //check switch 5
	players[1].points = 0;
	players[1].paddle.y = PADDLE2_Y;
	
	//set up player 1 sprites
	...
	while(1) {
		...
		//switch 1 alters wait time to poten 2
	...
	for(c = 0; c < NUM_BALLS; c++) {
		...
		if (b->y >= players[1].paddle.y) {
			...
			if (collX >= players[1].paddle.x && collX < players[1].paddle.x + paddleWidth(1)) {
				//speed increases logarithmically
				b->speed += .7;
				...
				//calculate new dx and dy and position
				b->dx = trueSpeed * sin(angle);
				b->dy = -trueSpeed * cos(angle);
				b->y = players[1].paddle.y + (1-percent)*b->dy;
				b->x = collX + (1-percent)*b->dx;

				//update last player to hit ball
				b->lastHit = &players[1];

				//update ai collision position
				updateColl(0);
				updateColl(1);
			} else {
				//paddle missed
...
/* count a score for player 1 */
void scoreP1() {
	players[0].points++;
	send();
	reset(TRUE);
}
/* count a score for player 2 */
void scoreP2() {
	players[1].points++;
	send();
	reset(FALSE);
		...
		if (powers[c].active) {
			for (d = 0; d < NUM_BALLS; d++) {
				if (balls[d].active) {
					...
					//collision detection
					//find the closest point the ball came to the powerup
...
void updatePaddle() {
	...
	for (c = 0; c < 2; c++) {
		if (players[c].ai) {
			//calculate new position based on target position and time
			...
			//use potentiometer to calculate position
...
void drawSprite(Sprite *s, float x, float y) {
	RIT128x96x4ImageDraw(s->data, (int)x - s->xOffset, (int)y - s->yOffset, s->width, s->height);
}

/* update collision information for ai players */
void updateColl(int player) {
	float pos;
	int minTime = 1000000;
	int minC = -1;
	...
	//find the ball which will hit this side the soonest
	for (c = 0; c < NUM_BALLS; c++) {
		if (balls[c].active) {
	...
	//account for reflection off of walls
	if (pos > 0) {
...
/* send updated data to scoreboard */
void send() {
	int i;
	int powers;
...
//{score_p1, score_p2, powerup_p1, powerup_p2, extra_p1, extra_p2};
	...
	powers = 0;
	//generate a bitmask from the powers
...
/* get the width of the given player's paddle */
int paddleWidth(int player) {
	return paddleSprite(player)->width;
}

/* get the sprite for the given player */
Sprite *paddleSprite(int player) {
	//use powerups to determine which paddle to use
...
/* clamp a into the range [cl, ch] */
float clamp(float a, float cl, float ch) {
	return max(cl, min(a, ch));
}
/* linearly interpolate between a and b with mix factor z */
float mix(float a, float b, float z) {
	return a*(1-z) + b*z;
}
